...
| puppeteer on Windows | `--no-sandbox` is now gated: auto-enabled on `process.platform === 'win32'` or when `PUPPETEER_NO_SANDBOX=true` env var is set. Set the env var on Linux hosts that don't support user namespaces (Railway, Render). Puppeteer downloads Chromium on first `pnpm install` (~200 MB). |
